<#
  .SYNOPSIS
    Copies the default production .Net Framework ruleset to all .Net Core projects under the test directory.
  .EXAMPLE
    .\Copy-DefaultTestRuleset-NetFx-ToProjets.ps1
  .INPUTS
    (NONE)
  .OUTPUTS
    (NONE)
...
process
{
  try
  {
    ...
    {
      $getTestProjectScript = Join-Path -Path $buildDirectory -ChildPath 'Get-TestCSharpProjectsNetFx.ps1'
      $testProjects = & $getTestProjectScript
