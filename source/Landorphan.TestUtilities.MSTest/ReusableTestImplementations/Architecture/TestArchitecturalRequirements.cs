namespace Landorphan.TestUtilities.ReusableTestImplementations
{
    ...
    public abstract class TestArchitecturalRequirements : TestBase
    {
        ...
        protected void All_But_Excluded_Tests_Descend_From_TestBase_Implementation()
        {
            ...
            {
                throw new AssertFailedException(string.Join(Environment.NewLine, failureMessages.ToArray()));
            }
        }
                        ...
                        switch (timingMatches)
                        {
                            ...
                            case 0:
                                failureMessages.Add(
                                    string.Format(
                                        CultureInfo.InvariantCulture,
                                        "The test '{0}.{1}' is not decorated with [TestCategory(TestTiming.*)] but should be.",
                                        testClass.Name,
                                        testMethod));
                                break;
                            ...
                            default:
                                failureMessages.Add(
                                    string.Format(
                                        CultureInfo.InvariantCulture,
                                        "The test '{0}.{1}' is decorated with more than one [TestCategory(TestTiming.*)] but should only have one timing attribute.",
                                        testClass.Name,
                                        testMethod));
                                break;
                        }
