namespace Landorphan.Common.Decorators
{
   ...
   /// <summary>
   /// Represents a <see cref="RankDecorator{TEntity,TRank}"/> comparison that evaluates both the <see cref="RankDecorator{TEntity,TRank}.Rank"/> values,
   /// and the
   ...
   /// </typeparam>
   [Serializable]
   public sealed class RankDecoratorDeepComparer<TEntity, TRank> : IComparer<RankDecorator<TEntity, TRank>>
      where TEntity : class, IComparable<TEntity>
      where TRank : IComparable<TRank>, IEquatable<TRank>, new()
   {
      ...
      /// </returns>
      /// <remarks>
      /// Compares by <see cref="RankDecorator{TEntity,TRank}.Rank"/> value and when equivalent, compares the decorated instances.
      /// </remarks>
      public Int32 Compare(RankDecorator<TEntity, TRank> x, RankDecorator<TEntity, TRank> y)
      {
         ...
         if (rv == 0)
         {
            if (x.Value.IsNull())
            {
               rv = y.Value.IsNull() ? 0 : -1;
            }
            else if (y.Value.IsNull())
            {
               return 1;
            }
            else
            {
               rv = x.Value.CompareTo(y.Value);
            }
         }
